...
img=imread('E:/6Mabing/计算机视觉论文/分层贝叶斯模型进行场景分类/Phoning_0001.jpg');
...
for i=1:step:m-mod(m,step)          %如果处理的m/step不是整数，是整数也可以，因为mod求余为0
    ii=1;
    for j=1:step:n-mod(n,step)      %注释同上
        tmpx=Ix(i:i+step-1,j:j+step-1);  % 取一个元胞梯度Gx
        tmped=Ied(i:i+step-1,j:j+step-1);  % 取一个元胞梯度值
        tmped=tmped/sum(sum(tmped));        %局部边缘强度归一化，即加权权值
        tmpphase=Iphase(i:i+step-1,j:j+step-1); % 取一个元胞的梯度方向

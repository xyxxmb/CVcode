...
if(dictionary_flag)
    ...
    inFName = fullfile(image_dir, sprintf('%s', 'hog_features'));
    load(inFName, 'features');
    ...
    for n=1:niters
        ...
        for f = 1:nimages  % 240张训练集图片参与聚类
            ...
            inFName = fullfile(image_dir, sprintf('%s',featureName));  % 取第f张图片的hog特征矩阵
                   
            load(inFName, 'features');
            data = features.data;
            [ndata, data_dim] = size(data); % 第一个hog特征矩阵的行与列

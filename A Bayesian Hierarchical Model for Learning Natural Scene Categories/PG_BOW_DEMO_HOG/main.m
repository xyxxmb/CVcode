...
%% Descriptors （Hog Features）
descriptor_opts.type='hog';     % name descripto
descriptor_opts.name=['des',descriptor_opts.type]; % output name (combines detector and descrtiptor name)
descriptor_opts.cellSize=8;   % cell大小
descriptor_opts.maxImageSize=1000;
GenerateHogDescriptors(pg_opts,descriptor_opts);  % 产生每张图片的x*36维的hog特征，共计300张图片

%% Create the texton dictionary
dictionary_opts.dictionarySize = 300;
dictionary_opts.name='hog_features';
dictionary_opts.type='hog_dictionary';
...
% 该词袋模型BOW存储在BOW_sift.mat文件下，需要用load加载BOW_sift.mat，才可以在Workspace显示词袋模型BOW
